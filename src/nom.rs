...
fn whitespace0(i: &str) -> IResult<&str, ()> {
    let res = tuple((multispace0, many0(pair(comments, multispace0)), multispace0))(i)?;
    Ok((res.0, ()))
}

fn whitespace1(i: &str) -> IResult<&str, ()> {
    let res = alt((
        tuple((multispace1, many0(pair(comments, multispace0)), multispace0)),
        tuple((multispace0, many0(pair(comments, multispace0)), multispace1)),
    ))(i)?;
    Ok((res.0, ()))
}
    ...
    alt((
        ...
        map(
            ...
            |(_, s)| {
                format!(
                    "xor{}",
                    if let Some((_, _, _, a, _, _, _, b, _, _)) = s {
                        format!("({}-{})", a, b)
                    } else {
                        "".to_string()
                    }
...
fn name(input: &str) -> IResult<&str, String> {
    ...
    Ok((
        res.0,
        format!("{}{}", res.1 .0, res.1 .1.iter().collect::<String>()),
    ))
}
...
fn string_count2(input: &str) -> IResult<&str, String> {
    ...
    Ok((
        ...
        format!(
            "@{}{}",
            res.1 .1.iter().collect::<String>(),
            if let Some((_, _, rr, _, _)) = res.1 .2 {
                format!("[{}]", rr)
            } else {
                "".to_string()
            }
    ...
    let res = tuple((
        ...
        many1(tuple((
            ...
            opt(delimited(
                ...
                opt(pair(
                    preceded(whitespace0, condition),
                    many0(preceded(
                        tuple((whitespace0, tag(","), whitespace0)),
                        condition,
                    )),
...
fn import(i: &str) -> IResult<&str, crate::YarImport> {
    let res = tuple((whitespace0, tag("import"), whitespace1, string))(i)?;
    Ok((res.0, crate::YarImport { value: res.1 .3 }))
}

fn include(i: &str) -> IResult<&str, crate::YarInclude> {
    let res = tuple((whitespace0, tag("include"), whitespace1, string))(i)?;
    Ok((
        res.0,
        crate::YarInclude {
            value: res.1 .3[1..res.1 .3.len() - 1].to_string(),
        },
    ))
}
...
fn parens(i: &str) -> IResult<&str, crate::YarRuleConditionNode> {
    preceded(
        ...
        delimited(
            tag("("),
            delimited(whitespace0, condition, whitespace0),
            tag(")"),
        ),
    )(i)
}
...
fn bytes_with_offset(i: &str) -> IResult<&str, crate::YarRuleConditionNode> {
    ...
    Ok((
        res.0,
        crate::YarRuleConditionNode::BytesWithOffset(res.1 .1.to_string(), Box::new(res.1 .5)),
    ))
}
...
fn size(i: &str) -> IResult<&str, usize> {
    ...
    Ok((
        res.0,
        res.1 .0 * if res.1 .1 == "KB" { 1024 } else { 1024 * 1024 },
    ))
}
...
fn range(i: &str) -> IResult<&str, crate::YarRuleConditionNode> {
    ...
    Ok((
        res.0,
        crate::YarRuleConditionNode::Range(Box::new(res.1 .2), Box::new(res.1 .6)),
    ))
}
...
fn not(i: &str) -> IResult<&str, crate::YarRuleConditionNode> {
    let res = tuple((tag("not"), whitespace1, condition))(i)?;
...
fn for_of(i: &str) -> IResult<&str, crate::YarRuleConditionNode> {
    ...
    Ok((
        res.0,
        crate::YarRuleConditionNode::ForOf(
            Box::new(res.1 .3),
            Box::new(res.1 .7),
            Box::new(res.1 .12),
        ),
...
fn for_in(i: &str) -> IResult<&str, crate::YarRuleConditionNode> {
    ...
    Ok((
        res.0,
        crate::YarRuleConditionNode::ForIn(
            Box::new(res.1 .3),
            res.1 .5,
            Box::new(res.1 .9),
            Box::new(res.1 .14),
    ...
    let res = tuple((
        ...
        opt(tuple((
            ...
            many1(tuple((
                ...
                tag("="),
                whitespace0,
                alt((hexdecimal_string, string, regex)),
            ))),
        ))),
        whitespace0,
        tag("condition"),
...
pub fn parse_rules(f: String, i: &str) -> IResult<&str, crate::YarRuleSet> {
    ...
    Ok((res.0, crate::YarRuleSet::new(f, includes, imports, rules)))
}
